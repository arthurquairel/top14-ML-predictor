...
print(f"Stats chargées pour {len(df_stats)} équipes.")
...
# ==========================================
# 3. FONCTION DE PRÉPARATION (FUSION)
# ==========================================
...
# ==========================================
# 4. MODÈLE DEEP LEARNING (BATCHNORM + DROPOUT)
# ==========================================
...
print("\n🔮 --- PRÉDICTIONS TOP 14 (Prochaine Journée) ---")
...
"""##Prédiction du classement final du top 14

"""

# ==========================================
# 1. PARAMÈTRES ET POINTS ACTUELS
# ==========================================
...
team_map = {
    "Bordeaux Bègles": "UBB", "Toulon": "RCT", "Montpellier": "MHR",
    "Montauban": "USM", "Clermont": "ASM", "Stade Français": "SFR",
...
def nettoyer(val):
    if isinstance(val, str):
        ...
        # Parfois il reste des caractères invisibles
        try:
            return float(val)
        except:
            return 0.0
...
cols_to_clean = [
    ...
    'plaquage_def', 'Offloads', 'essai_att', 'Penalité_att',
    'franchis_att', 'plaquage_att', 'Turnover_def', 'penalité_deff'
]
...
for col in cols_totaux:
    if col in df_stats.columns:
        # On crée une nouvelle colonne "_avg"
        df_stats[col + '_avg'] = df_stats[col] / df_stats['Matchs']

print("Stats avancées calculées (Offloads/match, Mètres/match, etc.)")
...
Castres;Clermont|Montpellier;Stade Français|Racing 92;Perpignan|Pau;Toulon|La Rochelle;Lyon|Toulouse;Bayonne|Montauban;Bordeaux Bègles
Bayonne;Racing 92|Lyon;Montauban|Toulon;Clermont|Stade Français;Toulouse|La Rochelle;Montpellier|Bordeaux Bègles;Castres|Perpignan;Pau
Clermont;Bayonne|Castres;La Rochelle|Lyon;Toulon|Montpellier;Racing 92|Pau;Bordeaux Bègles|Stade Français;Perpignan|Toulouse;Montauban
Clermont;Montpellier|Racing 92;Castres|Toulon;Stade Français|La Rochelle;Pau|Bordeaux Bègles;Toulouse|Montauban;Bayonne|Perpignan;Lyon
Bayonne;La Rochelle|Castres;Montauban|Lyon;Bordeaux Bègles|Pau;Racing 92|Stade Français;Clermont|Toulouse;Montpellier|Perpignan;Toulon
Clermont;Lyon|Bayonne;Pau|Castres;Toulouse|Montpellier;Perpignan|Racing 92;Stade Français|La Rochelle;Bordeaux Bègles|Montauban;Toulon
Lyon;Castres|Toulon;Bayonne|Stade Français;Pau|Toulouse;Clermont|Bordeaux Bègles;Montpellier|Montauban;Racing 92|Perpignan;La Rochelle
Clermont;Perpignan|Bayonne;Bordeaux Bègles|Montpellier;Montauban|Racing 92;La Rochelle|Toulon;Toulouse|Pau;Castres|Stade Français;Lyon
Castres;Montpellier|Lyon;Bayonne|Racing 92;Toulon|Pau;Clermont|La Rochelle;Toulouse|Bordeaux Bègles;Perpignan|Montauban;Stade Français
Clermont;Racing 92|Montpellier;Pau|Toulon;Bordeaux Bègles|Stade Français;Bayonne|Toulouse;Lyon|Montauban;La Rochelle|Perpignan;Castres
Bayonne;Perpignan|Castres;Toulon|Lyon;Montpellier|Racing 92;Toulouse|Pau;Montauban|La Rochelle;Stade Français|Bordeaux Bègles;Clermont
"""
future_matches = []
...
def prepare_data(matches_list, stats_df, is_training=True):
    ...
    df['Diff_Points_Inverse'] = df['P_def_home_Home'] - df['P_att_away_Visitor'] # Plus c'est haut, meilleure est la défense dom

    # 2. Jeu courant (Mètres & Offloads)
    # Comparaison de l'attaque Dom vs le "niveau moyen" des équipes
    ...
    # 3. Réalisme & Discipline
    df['Diff_Essais_Marques'] = df['essai_att_avg_Home'] - df['essai_att_avg_Visitor']
    df['Diff_Penalites_Concedees'] = df['penalité_deff_avg_Home'] - df['penalité_deff_avg_Visitor'] # Négatif = Mieux (moins de fautes)
...
class AdvancedPredictor(nn.Module):
    def __init__(self, input_size):
        ...
        self.fc4 = nn.Linear(16, 1) # Sortie : Écart de points

        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(0.27) # Dropout fort pour éviter le par coeur
...
print("Step 2: Entraînement du modèle avancé...")
epochs = 125
train_losses = []
val_losses = []
for epoch in range(epochs):
...
for i, (team, pts) in enumerate(sorted_rank, 1):
    qualif = ""
    if i <= 2: qualif = "✅ Demies"
    elif i <= 6: qualif = "🛡️ Barrages"
    elif i == 13: qualif = "⚠️ Barrage Maintien"
    elif i == 14: qualif = "🔻 Relégation"
    print(f"{i}. {team} : {pts} pts {qualif}")




print("\n🏉 --- DÉTAIL DES MATCHS À VENIR ---")
print(f"{'DOMICILE':<15} vs {'EXTÉRIEUR':<15} | {'VAINQUEUR PRÉVU':<15} | {'SCORE ESTIMÉ'}")
print("-" * 75)

for index, row in df_future.iterrows():
    h = row['Home']
    v = row['Visitor']
    ecart = row['Pred_Ecart'] # Positif = Dom gagne, Négatif = Ext gagne

    # Logique d'affichage
    if ecart > 0:
        vainqueur = h
        phrase_score = f"+{abs(ecart):.0f} pts"
        # On ajoute une petite étoile si c'est une grosse victoire (Bonus Offensif probable)
